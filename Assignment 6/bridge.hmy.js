...
/// HarmonyProver receives hash of burn transaction and generates proof-of-burn
/// when user wants to unlock their tokens on Ethereum
...
describe("HarmonyProver", function () {
  ...
  /// testing if we can get proof-of-lock from EProver with tx hash
  /// and parse proof header in HProver into BlockHeader data structure and proof data
  it("parse transaction receipt proof", async function () {
    ...
    let isTxn = true;
    /// using tx hash, we get the block header
    /// where the tx was included via rpc to interact with Ethereum client
    /// the header is passed into the arguments of getReceiptProof as proof header data,
    /// and the rest of the proof is retrieved from EProver
