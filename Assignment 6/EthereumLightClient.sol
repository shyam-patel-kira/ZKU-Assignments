...
contract EthereumLightClient is Ethash, Initializable, PausableUpgradeable {
    ...
    // the initialize function takes one input _rlpHeader and finalityConfirms this value with DEFAULT_FINALITY_CONFIRMS constant value
    // _rlpHeader is passed into the calcBlockHeaderHash function in the EthereumParser library a unit is returned
    // _rlpHeader is passed into the parseBlockHeader function in the EthereumParser library and a BlockHeader is returned
    // a StoredBlockHeader struct named storedBlock is created and uodated with the returned BlockHeader data
    // the _setFirstBlock function is called with storedBlockas input
    ...
    // the getBlockHeightMax function returns blockHeightMax
    function getBlockHeightMax() public view returns (uint256) {
        return blockHeightMax;
    }

    // the getStateRoot function takes as input a blockHash
    // gets the BlockHeader of the block hash from the blocks mapping
    // and retuns the bytes32 of the BlockHeader's stateroot
    function getStateRoot(bytes32 blockHash) public view returns (bytes32) {
        return bytes32(blocks[uint256(blockHash)].stateRoot);
    }

    // the getTxRoot function takes as input a blockHash
    // gets the BlockHeader of the block hash from the blocks mapping
    // and retuns the bytes32 of the BlockHeader's transactionsRoot
    function getTxRoot(bytes32 blockHash) public view returns (bytes32) {
        return bytes32(blocks[uint256(blockHash)].transactionsRoot);
    }

    // the getReceiptRoot function takes as input a blockHash
    // gets the BlockHeader of the block hash from the blocks mapping
    // and retuns the bytes32 of the BlockHeader's receiptsRoot
    ...
    // the VerifyReceiptsHash function takes as input a blockHash and receiptsHash
    // gets the BlockHeader of the block hash from the blocks mapping
    // and checks if the  receiptsHash is the same as BlockHeader's receiptsRoot and retuns a bool
    ...
    // the _setFirstBlock functions take as input the StoredBlockHeader struct
    // updates some publci mappings with the value
