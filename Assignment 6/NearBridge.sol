...
contract NearBridge is INearBridge, AdminControlled {
    using Borsh for Borsh.Data; // Borsh is a serialization format that is used to serialize/deserialize NEAR block headers
    ...
    struct Epoch {
        bytes32 epochId; // the epoch id is generally the hash of the last block in the epoch before it
        uint256 numBPs; // number of block producers
        bytes32[MAX_BLOCK_PRODUCERS] keys; // the public keys of the block producers
    ...
    // the object we will use when dealing with the Ed25519 signature scheme
    ...
    // Declare some variables related to the epoch
    Epoch[3] epochs;
    uint256 curEpoch;
    ...
    constructor(
        Ed25519 ed, // this is a contract with data and instructions related to the Ed25519 signature scheme
        uint256 lockEthAmount_,
        uint256 lockDuration_,
        uint256 replaceDuration_,
        address admin_, // address of the admin
        uint256 pausedFlags_ // set of bits that indicate which functions are paused or not
    ) AdminControlled(admin_, pausedFlags_) {
        require(replaceDuration_ > lockDuration_ * 1000000000);
        edwards = ed;
        lockEthAmount = lockEthAmount_; // set the amount we're allowing to be locked
        lockDuration = lockDuration_; // set how long we want the lock duration to be
        replaceDuration = replaceDuration_;
    }

    // various flags for the different functions we might want to pause
    // notice they are all powers of 2
    uint256 constant UNPAUSE_ALL = 0;
    uint256 constant PAUSED_DEPOSIT = 1;
    uint256 constant PAUSED_WITHDRAW = 2;
    uint256 constant PAUSED_ADD_BLOCK = 4;
    uint256 constant PAUSED_CHALLENGE = 8;
    uint256 constant PAUSED_VERIFY = 16;

    // deposit ETH into the contract and lock it up
    function deposit() public payable override pausable(PAUSED_DEPOSIT) {
        // ensure the ETH being sent is equal to the amount we're allowing to be locked
    ...
    // this function is called when a user or the watchdog service believes that the block producer signature of a
    // NEAR block header is invalid
    ...
    // check if the signature of the block producer is valid or not
        ...
        returns (bool)
    {
        ...
        require(
            (untrustedSignatureSet & (1 << signatureIndex)) != 0,
            "No such signature"
        );
        unchecked {
            Epoch storage untrustedEpoch = epochs[
                untrustedNextEpoch ? (curEpoch + 1) % 3 : curEpoch
            ];
            NearDecoder.Signature storage signature = untrustedSignatures[
                signatureIndex
            ];
            ...
            return
                edwards.check(
                    untrustedEpoch.keys[signatureIndex],
                    signature.r,
                    signature.s,
                    arg1,
                    arg2
    ...
    // Declare a struct containing data about the Bridge's state
    struct BridgeState {
        ...
        uint256 nextTimestamp; // Timestamp of the current unconfirmed block
        uint256 nextValidAt; // Timestamp when the current unconfirmed block will be confirmed
        uint256 numBlockProducers; // Number of block producers for the current unconfirmed block
    }

    // This function returns the current state of the bridge
    ...
    // This function essentially verifies then adds a NEAR block header
    // (note: it does not verify that the Ed25519 signature is valid)
    function addLightClientBlock(bytes memory data)
        public
        override
        pausable(PAUSED_ADD_BLOCK)
    {
        ...
        unchecked {
            ...
            if (block.timestamp < lastValidAt) {
                require(
                    nearBlock.inner_lite.timestamp >=
                        untrustedTimestamp + replaceDuration,
    ...
    // sets the public keys of the block producers for the current epoch and updates variables related to their stake
    ...
    // returns the hash of the block at the provided height
    ...
    // returns the merkle root hash of the block at the provided height
