...
/**
 * Memory pool is simple in memory buffer to store zkSync transactions and runs inside server application.
 * It accepts transactions from api, check signatures and basic nonce correctness (nonce not too small).
 * Transactions are stored in MempoolTransactionsQueue.
 * It manages 4 in memory transaction queues:
**/
#[derive(Debug, Clone)]
pub struct MempoolTransactionsQueue {
    ...
    /// 1. reverted_txs: 
    /// reverted transactions queue that must be used before processing any other transactions.
    /// Most of the time it will be empty unless the server is restarted after reverting blocks.
    /// The queue is only accessible for popping elements.
    ...
    /// 4. priority_ops: 
    /// Queue to store priority transactions, like deposit, withdraw.
